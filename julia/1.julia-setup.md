...
Due to some special reasons, we need to install `PkgServerClient.jl` first. `PkgServerClient.jl` is a Julia package for choosing the package mirror that will provide the best downloading speed for you. You should first execute the following command in your shell. It will have a one session only effect of using mirror from Nan Jing University to download Julia packages.

  **Linux and macOS**
  ```bash
    export JULIA_PKG_SERVER='https://mirrors.nju.edu.cn/julia' # Linux & macOS
  ```

  **Windows**
  ```PowerShell
    $env:JULIA_PKG_SERVER='https://mirrors.nju.edu.cn/julia' # Windows
  ```

Then you can add `PkgServerClient.jl` by typing `add PkgServerClient` in the package manager. To make sure Julia uses the package automatically at startup, configure the `startup.jl` file by adding the following block.
